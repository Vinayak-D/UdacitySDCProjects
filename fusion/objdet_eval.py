...
import numpy as np
import matplotlib
matplotlib.use('Qt5agg') # change backend so that figure maximizing works on Mac as well     
...
def measure_detection_performance(detections, labels, labels_valid, configs, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            fl_lbl, rl_lbl, rr_lbl, fr_lbl = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length , label.box.heading)
            
            ## step 2 : loop over all detected objects - compare each valid label with all detections
            for d in detections:
                ## step 3 : extract the four corners of the current detection
                if configs.use_labels_as_objects == False:
                    fl_det, rl_det, rr_det, fr_det = tools.compute_box_corners(d[1].item(), d[2].item(), d[5].item(), d[6].item(), d[7].item())
                    ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                    dist_x = np.abs(label.box.center_x - d[1].item())
                    dist_y = np.abs(label.box.center_y - d[2].item())
                    dist_z = np.abs(label.box.center_z - d[3])
                else:
                    fl_det, rl_det, rr_det, fr_det = tools.compute_box_corners(d[1], d[2], d[5], d[6], d[7])
                    ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                    dist_x = np.abs(label.box.center_x - d[1])
                    dist_y = np.abs(label.box.center_y - d[2])
                    dist_z = np.abs(label.box.center_z - d[3])
                    
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_poly = Polygon([fl_lbl, rl_lbl, rr_lbl, fr_lbl])
                d_poly = Polygon([fl_det, rl_det, rr_det, fr_det])
                intersect = label_poly.intersection(d_poly).area
                union = label_poly.union(d_poly).area
                iou = intersect/union
...
def compute_performance_stats(det_performance_all, configs_det):
    ...
    all_positives = item[2][0]
    true_positives = item[2][1]
    false_negatives = item[2][2]
    false_positives = item[2][3]
