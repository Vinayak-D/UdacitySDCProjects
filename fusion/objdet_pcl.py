...
def show_pcl(pcl):
    ...
    # step 4 : for all frames, add the pcd instance to visualization using add_geometry
    vis.add_geometry(pcd)
        
    # step 5 : visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    #all key codes: https://www.glfw.org/docs/latest/group__keys.html
...
def show_range_image(frame, lidar_name):
    ...
    # focus on +/- 90 deg around the image center
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    scaling = (configs.lim_x[1] - configs.lim_x[0])/configs.bev_height

    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates    
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:,0] = np.int_(np.floor(lidar_pcl_cpy[:,0]/scaling))
    
    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    offset = (configs.bev_width + 1) / 2
    lidar_pcl_cpy[:,1] = np.int_(np.floor(lidar_pcl_cpy[:,1]/scaling) + offset)
    ...
    counts = np.zeros([configs.bev_height + 1, configs.bev_width + 1])
    temporary = lidar_pcl_top[0:,0:2]
    c = 1
    for i in range(lidar_pcl_top.shape[0] - 1):
        if np.array_equal(temporary[i+1], temporary[i]):
            c = c + 1
        else:
            xcoord = int(temporary[i][0])
            ycoord = int(temporary[i][1])
            counts[(xcoord,ycoord)] = c
            c = 1
    ...
    scaling2 = np.amax(lidar_pcl_top[:, 3]) - np.amin(lidar_pcl_top[:, 3])
    intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 3] / scaling2
    ...
    scaling3 = float(np.abs(configs.lim_z[1] - configs.lim_z[0]))
    height_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = lidar_pcl_top[:, 2] / scaling3
