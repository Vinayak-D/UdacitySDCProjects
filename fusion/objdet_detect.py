...
def detect_objects(input_bev_maps, model, configs):
    ...
    with torch.no_grad():  
        ...
        if 'darknet' in configs.arch:
            ...
            decoded_outputs = decode(outputs["hm_cen"], outputs["cen_offset"], outputs["direction"], outputs["z_coor"], outputs["dim"], 40)
            output_post = post_processing(decoded_outputs, configs)
            detections = []
            for sample_i in range(len(output_post[0])):
                if int(np.sum(output_post[0][sample_i])) == 0:
                    continue
                for j in range(output_post[0][sample_i].shape[0]):
                    values = output_post[0][sample_i][j]
                    #extract each value
                    score = output_post[0][sample_i][j][0]
                    x = output_post[0][sample_i][j][1]
                    y = output_post[0][sample_i][j][2]
                    z = output_post[0][sample_i][j][3]
                    h = output_post[0][sample_i][j][4]
                    w = output_post[0][sample_i][j][5]
                    l = output_post[0][sample_i][j][6]
                    yaw = output_post[0][sample_i][j][7] 
                    detections.append(np.array([score, x, y, z, h, w, l, yaw]))      
            #######
            ####### ID_S3_EX1-5 END #######     
    ...
    if len(detections) > 0:
        ...
        for detection in detections:
            ...
            _score, _x, _y, z, h, _w, _l, yaw = detection
            scalerXY = (configs.lim_x[1] - configs.lim_x[0])/configs.bev_height
            offset = (configs.lim_y[1] - configs.lim_y[0]) / 2.0
            detection[0] = 1
            detection[1] = _y*scalerXY
            detection[2] = _x*scalerXY - offset
            detection[5] = _w*scalerXY 
            detection[6] = _l*scalerXY 
            detection[7] = yaw
            ## step 4 : append the current object to the 'objects' array if it meets metric constraints
            if ((detection[1] >= configs.lim_x[0]) and (detection[1]  <= configs.lim_x[1]) and (detection[2] >= configs.lim_y[0])
                and (detection[2] <= configs.lim_y[1]) and (detection[3] >= configs.lim_z[0]) and (detection[3] <= configs.lim_z[1])):
