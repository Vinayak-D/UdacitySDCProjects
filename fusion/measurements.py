...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        # otherwise False.
        ############
        x_pos = np.array([[x[0].item()],[x[1].item()],[x[2].item()],[1]])
        x_pos_sens = self.veh_to_sens @ x_pos
        
        if x_pos_sens[0] > 0:
            angle = np.arctan(abs(x_pos_sens[1].item()/x_pos_sens[0].item()))
            if angle < self.fov[1]:   
                return True
        return False
        ############
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            pos_veh[0:3] = x[0:3] 
            pos_sens = self.veh_to_sens*pos_veh # same values, since measurements already in vehicle coordinates
            return pos_sens[0:3] #3x1 product H*x
            ...
            pos_veh = np.ones((4, 1))
            pos_veh[0:3] = x[0:3] 
            pos_sens = self.veh_to_sens*pos_veh # from vehicle to camera coordinates (not the same anymore)
            if pos_sens[0] == 0:
                raise NameError ("Div by 0!")
            else:
                img_coords = np.zeros((2, 1))
                x_img = self.c_i - (self.f_i*pos_sens[1])/pos_sens[0]
                y_img = self.c_j - (self.f_j*pos_sens[2])/pos_sens[0]
                img_coords[0] = x_img
                img_coords[1] = y_img
            return img_coords #2x1 product H*x, no multiplication here because lines 84/85 directly solve H*x, H is nonlinear in this case 
            ...
            if R[0,0]*x[0] + R[0,1]*x[1] + R[0,2]*x[2] + T[0] == 0: 
                ...
                H[1,2] = self.f_j * (-R[2,2] / (R[0,0]*x[0] + R[0,1]*x[1] + R[0,2]*x[2] + T[0])
                                    + R[0,2] * (R[2,0]*x[0] + R[2,1]*x[1] + R[2,2]*x[2] + T[2]) \
                                        / ((R[0,0]*x[0] + R[0,1]*x[1] + R[0,2]*x[2] + T[0])**2))
        return H #nx6, for 6 states, n = 2 for camera and 3 for LIDAR
