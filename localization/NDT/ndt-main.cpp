...
double Probability(Eigen::MatrixXd X, Eigen::MatrixXd Q, Eigen::MatrixXd S){
	// TODO: calculate the probability of the point given mean and standard deviation
	Eigen::MatrixXd Sinv = S.inverse();
	Eigen::MatrixXd dXQ = X-Q;
	return exp( -0.5 * (dXQ.transpose() * Sinv * dXQ)(0,0) );;
}
...
Cell PDF(PointCloudT::Ptr input, int res, pcl::visualization::PCLVisualizer::Ptr& viewer){
	...
	Q(0,0) = Q(0,0) / input->points.size();
	Q(1,0) = Q(1,0) / input->points.size();
	// TODO: Calculate the 2 x 2 matrix S, which is covariance of the input points
	Eigen::MatrixXd S(2,2);
	S << Eigen::MatrixXd::Zero(2,2);
	for(PointT point : input->points){
		Eigen::MatrixXd X(2,1);
	...
	S(1,0) = S(1,0)/input->points.size();
	S(1,1) = S(1,1)/input->points.size();
	//The cell will be a region from 0 to 10 in both x and y directions and contain a set of five contrived points.
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived>& g_previous, Eigen:: MatrixBase<Derived>& H_previous){
	...
	Eigen::MatrixXd q = X - cell.Q;

	// TODO: calculate the 3 2 x 1 partial derivative matrices
	// each with respect to x, y, and theta
	Eigen::MatrixXd q1p1(2,1);
	q1p1 << Eigen::MatrixXd::Zero(2,1);
	q1p1(0,0) = 1.0;
	Eigen::MatrixXd q1p2(2,1);
	q1p2 << Eigen::MatrixXd::Zero(2,1);
	q1p2(1,0) = 1.0;
	Eigen::MatrixXd q1p3(2,1);
	q1p3(0,0) = -point.x*sin(theta) - point.y*cos(theta);
	q1p3(1,0) = point.x*cos(theta) - point.y*sin(theta);
	
	// TODO: calculate the 1 x 1 exponential matrix which uses q, and S inverse
	Eigen::MatrixXd score_i(1,1);
	score_i(0,0) = exp(-0.5 * (q.transpose() * Sinv * q)(0,0) );

	// TODO: calculate the matrix g which uses q, exponential, S inverse, and partial derivatives
	Eigen::MatrixXd g(3,1);
	g(0,0) = (q.transpose() * Sinv * q1p1 * score_i)(0,0);
	g(1,0) = (q.transpose() * Sinv * q1p2 * score_i)(0,0);
	g(2,0) = (q.transpose() * Sinv * q1p3 * score_i)(0,0);
    
    // TODO: calculate the 2 x 1 second order partial derivative matrix
	Eigen::MatrixXd q2p33(2,1);
	q2p33(0,0) = -point.x*cos(theta) + point.y*sin(theta);
	q2p33(1,0) = -point.x*sin(theta) - point.y*cos(theta);
	...
	H(0,0) = (-score_i*( (-q.transpose()*Sinv*q1p1)*(-q.transpose()*Sinv*q1p1)+(-q1p1.transpose()*Sinv*q1p1)))(0,0);
	H(0,1) = (-score_i*( (-q.transpose()*Sinv*q1p1)*(-q.transpose()*Sinv*q1p2)+(-q1p2.transpose()*Sinv*q1p1)))(0,0);
	H(0,2) = (-score_i*( (-q.transpose()*Sinv*q1p1)*(-q.transpose()*Sinv*q1p3)+(-q1p3.transpose()*Sinv*q1p1)))(0,0);

	H(1,0) = (-score_i*( (-q.transpose()*Sinv*q1p2)*(-q.transpose()*Sinv*q1p1)+(-q1p1.transpose()*Sinv*q1p2)))(0,0);
	H(1,1) = (-score_i*( (-q.transpose()*Sinv*q1p2)*(-q.transpose()*Sinv*q1p2)+(-q1p2.transpose()*Sinv*q1p2)))(0,0);
	H(1,2) = (-score_i*( (-q.transpose()*Sinv*q1p2)*(-q.transpose()*Sinv*q1p3)+(-q1p3.transpose()*Sinv*q1p2)))(0,0);

	H(2,0) = (-score_i*( (-q.transpose()*Sinv*q1p3)*(-q.transpose()*Sinv*q1p1)+(-q1p1.transpose()*Sinv*q1p3)))(0,0);
	H(2,1) = (-score_i*( (-q.transpose()*Sinv*q1p3)*(-q.transpose()*Sinv*q1p2)+(-q1p2.transpose()*Sinv*q1p3)))(0,0);
	H(2,2) = (-score_i*( (-q.transpose()*Sinv*q1p3)*(-q.transpose()*Sinv*q1p3)+(-q.transpose()*Sinv*q2p33)+(-q1p3.transpose()*Sinv*q1p3)))(0,0);
...
int main(){
	...
	// Part 1: Visualize a single PDF cell
	bool part1 = false;
	if(part1){
